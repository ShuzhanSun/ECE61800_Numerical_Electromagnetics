function FDTD_3D_Waveguide_For_Plot_TM(mu_r, epsi_r, f_max, W, L, d, m, hfdtd, hanaly, ax3, initWithAnaly)
...
%         analy_TE(B,m,d,omega,mu,epsi,t_E,t_H,Ny*dy,Ny*dy,(Ny-.5)*dy,Z_Ex(:,Ny+1,:),Z_Hy(:,Ny+1,:),Z_Hz(:,Ny,:));
    ...
    addpoints(hfdtd,t_E,Ey(10,3,10));
    drawnow;
    
    %% compute the relative error
    [~,EyA,~,~,~,~] = ParallelPlateAnalyOnePointTM(B, m, d, omega, t_H, mu, epsi, 5*dy/2, 9*dz);
    % show analytical result:
    addpoints(hanaly,t_E,EyA);
    drawnow;

    imagesc(ax3, squeeze(Ez(round(Nx/2),:,:)))
